...
namespace Basin.Utils.CssToXpath {
    public class Tokenizer {
        ...
        public static Str TokenizeEscapedString(string input, ref int pos) {
            ...
            if (next == -1) {
                throw new SelectorSyntaxException($"Expected closing {quote} for string in: {input.Substring(start)}");
            }
        ...
        public static Symbol TokenizeSymbol(string input, ref int pos) {
            ...
            if (match.Index == pos) {
                throw new SelectorSyntaxException($"Unexpected symbol: {input[pos]} at {pos}");
            }
